...
public class MenuUI : MonoSingleton<MenuUI>
{
    ...
    [SerializeField] private GameObject _upgradesCanvas, _infoPanel, _settingsPanel;
    ...
    { 
        switch (true)
        {
            case true when _upgradesCanvas.activeSelf:
                return true;
            case true when _infoPanel.activeSelf:
                return true;
            case true when _settingsPanel.transform.localScale == Vector3.one:
                return true;
            default:
                return false;
        }
